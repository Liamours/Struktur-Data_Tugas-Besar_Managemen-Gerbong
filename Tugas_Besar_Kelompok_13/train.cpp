...
void insertGerbongDepan(listGerbong &G, adrGerbong &pG) {
    ...
    if (!checkEmptyGerbong(G, pG)) {
        delete pG;
        handle_menuInsert();
    } else if (searchGerbong(G, idGerbong(pG)) != NULL) {
...
adrPenumpang searchPenumpangSpesifik(adrGerbong pG, int id) {
    // I.S. menerima input pointer pada gerbong dan id penumpang yang ingin di cari
    // F.S. mengembalikan pointer kepada penumpang dengan id yang cocok dan berada pada gerbong yang di tunjuk pG
    // mengembalikan Null apabila tidak ditemukan penumpang dengan id yang cocok pada gerbong pG
    adrPenumpang P = firstPenumpang(pG);

    while (P != NULL && idPenumpang(P) != id) {
        P = nextPenumpang(P);
    }
...
void importTemplateTrain() {
    ...
    while (penumpangIndex < 25 && i < 10) {
        int penumpangPerGerbong = 10;
        int j = 0;

        while (penumpangIndex < 25 && j < penumpangPerGerbong) {
        ...
        if (penumpangIndex < 25 && i < 9) {
            tempG = nextGerbong(tempG);
        }
...
void handle_searchPenumpang() {
    ...
    if (pP != NULL) {
        while (pG != NULL && searchPenumpangSpesifik(pG, id) == NULL) {
            pG = nextGerbong(pG);
        }
        ...
        if (pG != NULL) {
            ...
            showPenumpang(pP);
            cout << string(60, '-') << endl;
            cout << "Berada di gerbong dengan ID: " << idGerbong(pG) << endl;
            cout << string(60, '-') << endl;
        } else {
            cout << "Penumpang tidak ditemukan di gerbong manapun." << endl;
        }
    } else {
